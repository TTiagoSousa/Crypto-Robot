...
def plot_chart(df):
    ...
    # df['mean_close'] = df['close'].rolling(window=2).mean()  # Adjust window size as needed
    # # Add mean curve to the figure
    # fig.add_trace(go.Scatter(x=df['timestamp'], y=df['mean_close'], mode='lines', name='Mean Close'), row=1, col=1)

    #pandas.DataFrame.ewm candle bar example
    df['ewm'] = df['close'].ewm(span=10, adjust=False).mean()
    # Add ewm curve to the figure
    fig.add_trace(go.Scatter(x=df['timestamp'], y=df['ewm'], mode='lines', name='EWM Close'), row=1, col=1)
